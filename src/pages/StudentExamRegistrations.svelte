<script>
    ...
    const registrableExamColumnNames = ['Id', 'Exam date', 'Location', 'Term', 'Course', 'Register'];

    const fetchRegisteredExams = (page) => {
        return httpService
            .withAuth()
            .request({ method: 'GET', url: `api/students/${$user.id}/exam-registrations?page=${page}`});
    }
    ...
    const examRegistrationsToRegisteredTableRow = ({ exam }) => [exam.id, exam.examDate, exam.location, exam?.term?.name, exam?.course];
    const examsToRegistableTableRow = (exam) => [exam.id, exam.examDate, exam.location, exam.term.name, exam.course, { examId: exam.id }];
    ...
    const handleRegister = async (examId) => {
        ...
        await httpService.withAuth()
            .request({ method: 'POST', url: 'api/exam-registrations', body: JSON.stringify(registration)});
...
<PrivateLayout>
    ...
    >
        <svelte:fragment slot="pages" let:pages>
            <Table columnNames={registeredExamsColumnNames} tableData={reducePagesToTableRows(pages, examRegistrationsToRegisteredTableRow)}>
    ...
    >
        <svelte:fragment slot="pages" let:pages>
            <Table columnNames={registrableExamColumnNames} tableData={reducePagesToTableRows(pages, examsToRegistableTableRow)}>
                <svelte:fragment slot="row" let:cellValue let:index>
                    {#if index === 4}
                        <td>
                            <Link
                    ...
                    {:else if index === 5}
                        <td>
                            <button class="btn btn-primary" on:click={() => handleRegister(cellValue.examId)}>
                                <i class="bi bi-pen"></i>
                            </button>
                        </td>
                    {:else}
